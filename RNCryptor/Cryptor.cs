    ...
    abstract public class Cryptor
    {
        ...
        protected byte[] generateHmac(PayloadComponents components, string password)
        {
            ...
            if (hmac_includesHeader)
                hmacMessage.AddRange(assembleHeader(components));
            ...
            {
                for (int i = hmac.Count; i < hmac_length; i++)
                {
                    hmac.Add(0x00);
                }
            }
        ...
        protected byte[] generateKey(byte[] salt, string password)
        {
            ...
            return pbkdf2.GetBytes(pbkdf2_keyLength);
        }

        protected byte[] encryptAesCtrLittleEndianNoPadding(byte[] plaintextBytes, byte[] key, byte[] iv)
        {
            byte[] counter = computeAesCtrLittleEndianCounter(plaintextBytes.Length, iv);
            byte[] encrypted = encryptAesEcbNoPadding(counter, key);
            return bitwiseXOR(plaintextBytes, encrypted);
        }
        ...
        private byte[] computeAesCtrLittleEndianCounter(int payloadLength, byte[] iv)
        {
            ...
            int blockCount = (int)Math.Ceiling(payloadLength / (decimal)iv.Length);
